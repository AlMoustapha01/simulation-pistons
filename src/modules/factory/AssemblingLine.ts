...
import { Asking } from "../question";
import { questionNumAxe, questionNumJupe, questionNumTete } from "../constant";
...
export default class AssemblingLine {
  ...
  private fillCartonWithType(numPieces: number, type: PieceType) {
    for (let i = 0; i < numPieces; i++) {
      // Creating a new Piece and adding it to the carton
      const piece = new Pieces(type);
      this.setPiece(piece);
    }
  }

  // Method to sort the Pieces in the carton by their type
  private sortPieces(carton: Pieces[]): {
    tete: Pieces[];
    jupe: Pieces[];
    axe: Pieces[];
  } {
  ...
  public async productionSimulationPerso() {
    ...
    const numTete = await Asking(questionNumTete);
    this.fillCartonWithType(parseInt(numTete), "tete");
    const numAxe = await Asking(questionNumAxe);
    this.fillCartonWithType(parseInt(numAxe), "axe");
    const numJupe = await Asking(questionNumJupe);
    this.fillCartonWithType(parseInt(numJupe), "jupe");

    const totalPiece = parseInt(numTete) + parseInt(numAxe) + parseInt(numJupe);

    // Checking that there are enough Pieces to assemble all the pistons
    if (!this.carton || this.carton.length < totalPiece) {
      throw new Error(
        "carton must contain enough pieces to assemble all the pistons"
      );
    }
    // Checking that the number of pistons is a positive integer
    if (!totalPiece || totalPiece < 1) {
      throw new Error("numPistons must be a positive integer");
    }
    // Logging the start of the simulation
    console.log(
      `Assembling ${totalPiece} pieces from a carton of ${this.carton.length} pieces`
    );

    // Sorting the Pieces in the carton by their type
    const { tete, jupe, axe } = this.sortPieces(this.carton);

    // Loop that simulates the assembly of the pistons
    while (pistonsAssembled < totalPiece) {
      ...
      if (tete.length > 0 && jupe.length > 0 && axe.length > 0) {
        ...
        for (const machine of this.machines) {
          ...
          if (!machine.getIsBroken()) {
            ...
            const processingTime = batch.map(
              (piece) => piece && machine.processPiece(piece)
            );
            ...
            if (Math.random() <= 0.25) {
              ...
              console.log(`${machine.getType()} is broken...`);
        ...
        if (brokenMachines.length > 0) {
          ...
          // Logging the machine which is broken
          console.log(`${machineEnPanne.getType()} is repair...`);
        ...
        console.log("Not enough pieces of each type to assemble a piston.");
        console.log(
          `inventory: ${tete.length} 'tete', ${axe.length} 'axe', ${jupe.length} 'jupe' `
        );
    ...
    console.log(
      `Finished assembling ${totalPiece} pistons in ${totalTime} minutes`
    );
    return totalTime;
  }
